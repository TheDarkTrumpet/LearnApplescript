				...
				try
					...
					with timeout of 30 seconds
						tell application "Image Events"
							...
							copy the dimensions of this_file to {image_width, image_height}
							if image_width > image_height then
								rotate this_file to angle 90
								set didRotate to true
							end if
							close this_file saving yes
						end tell
					end timeout
					if didRotate then set the thumbnail of this_item to this_path
				end try
			end if
		end repeat
